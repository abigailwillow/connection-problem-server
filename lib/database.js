...
function createTables(err) {
    if (err) { throw err; }
    db.run('CREATE TABLE IF NOT EXISTS Leaderboard (steamid INTEGER PRIMARY KEY, name TEXT NOT NULL, score REAL NOT NULL)');
}

exports.getScore = (steamid, callback) => {
    db.get('SELECT steamid, name, score FROM Leaderboard WHERE steamid=?', steamid, (err, row) => {
        if (err) { throw err; }
        row ? callback(row.steamid, row.name, row.score) : callback(false);
     });
}
